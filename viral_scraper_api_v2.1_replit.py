...
# TikTok: Intentar importar, pero no fallar si no está disponible
TIKTOK_AVAILABLE = False
try:
    from TikTokApi import TikTokApi
    import asyncio
    TIKTOK_AVAILABLE = True
    print("✅ TikTok scraping disponible")
except Exception as e:
    print(f"⚠️ TikTok scraping NO disponible: {e}")
    print("   La API funcionará solo con YouTube e Instagram")

# ============================================
# FUNCIÓN: SCRAPING TIKTOK REAL (Solo si disponible)
# ============================================
async def scrape_tiktok_real(hashtag, cantidad=10):
    """
    Scraping real de TikTok usando TikTokApi
    Solo funciona si TikTokApi está disponible
    """
    ...
    try:
        ...
        if username and password:
            try:
                ...
                print("✅ Login Instagram exitoso")
            except Exception as e:
                print(f"⚠️ Login Instagram falló: {e}")
        ...
        for post in hashtag_posts.get_posts():
            ...
            if post.is_video or post.typename == 'GraphVideo':
                try:
                    ...
                    print(f"⚠️ Error procesando post Instagram: {e}")
                    continue
...
def scrape_youtube_real(hashtag, cantidad=10):
    ...
    if not YOUTUBE_API_KEY:
        print("❌ YouTube API Key no configurada")
        return []
    ...
    try:
        ...
        for video in videos_response['items']:
            try:
                ...
                print(f"⚠️ Error procesando video YouTube: {e}")
                continue
...
def scrape():
    ...
    try:
        ...
        # Si TikTok no está disponible, removerlo de la lista
        if not TIKTOK_AVAILABLE and 'tiktok' in platforms:
            platforms.remove('tiktok')
            print("⚠️ TikTok solicitado pero no disponible, se omitirá")
        
        if not platforms:
            return jsonify({
                'error': 'No hay plataformas válidas disponibles',
                'valid_platforms': ['youtube', 'instagram'],
                'note': 'TikTok no está disponible en este entorno'
            }), 400
            ...
            try:
                ...
                if platform == 'tiktok':
                    if TIKTOK_AVAILABLE:
                        ...
                        errors['tiktok'] = 'TikTok scraping no disponible en Replit'
                ...
                if platform in ['instagram', 'youtube'] or (platform == 'tiktok' and TIKTOK_AVAILABLE):
        ...
        if errors:
            ...
            response['note'] = 'Algunas plataformas tuvieron errores pero otras funcionaron correctamente'
...
def health():
    ...
    return jsonify({
        ...
        'tiktok_status': 'available' if TIKTOK_AVAILABLE else 'not_available_in_replit'
    })
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print("🚀 VIRAL SCRAPER API v2.1 - Replit Optimized")
    print("="*60)
    print("\n📊 Estado de plataformas:")
    print(f"  YouTube: {'✅ Disponible' if YOUTUBE_API_KEY else '❌ API Key no configurada'}")
    print(f"  Instagram: {'✅ Disponible' if os.getenv('INSTAGRAM_USER') else '⚪ Opcional (no configurado)'}")
    print(f"  TikTok: {'✅ Disponible' if TIKTOK_AVAILABLE else '⚠️ No disponible en Replit'}")
    print("\n📋 Endpoints disponibles:")
    print("   POST /scrape - Scraping multi-plataforma")
    print("   POST /test - Datos de prueba")
    print("   GET /health - Health check")
    print("\n💡 Recomendación: Usa YouTube e Instagram para mejores resultados")
    print("="*60 + "\n")
